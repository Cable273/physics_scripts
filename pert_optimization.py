...
N_vals = np.arange(10,20,2)
optimal_coef = np.zeros(np.size(N_vals))
for count in range(0,np.size(N_vals,axis=0)):
    ...
    res = minimize_scalar(lambda coef: pert_opt_fidelity(coef),method="golden",bracket=(-0.1,0.1))
    # res = minimize(lambda coef: pert_opt_fidelity(coef),method="powell",x0=[0.1,0.1])
    print(res.x)
    optimal_coef[count] = res.x
np.save("N",N_vals)
np.save("ppxp,optimal_coef",optimal_coef)
