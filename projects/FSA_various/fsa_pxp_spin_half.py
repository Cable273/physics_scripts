...
S2 = L
m2=np.arange(-S2,S2)
couplings2 = np.power(S2*(S2+1)-m2*(m2+1),0.5)
...
print(couplings)
print(couplings2)
print(np.diag(H_fsa,1))

plt.plot(couplings,label="N/2 cube")
plt.plot(couplings2,label="N cube")
plt.plot(np.diag(H_fsa,1),label="FSA")
plt.legend()
plt.show()
