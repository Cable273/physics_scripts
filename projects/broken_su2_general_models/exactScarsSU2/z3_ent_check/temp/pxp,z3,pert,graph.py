...
H.sector.find_eig(k)
for n in range(0,np.size(H.sector.matrix(k),axis=0)):
    for m in range(0,np.size(H.sector.matrix(k),axis=0)):
        if np.abs(H.sector.matrix(k)[n,m]) <1e-5:
            H.sector.matrix(k)[n,m] = 0
    
eig_overlap(z,H,k).plot(tol=-10)
plt.show()
fidelity(z,H,"use sym").plot(np.arange(0,20,0.01),z)
plt.show()

# from Calculations import gen_krylov_basis
# kbasis = gen_krylov_basis(H.sector.matrix(k),pxp.dim,z.prod_basis())
# print("krylov dim |z3>:")
...
plot_adjacency_graph(np.abs(H.sector.matrix(k)),labels=None,largest_comp=False)
plt.show()
