...
# coef = np.zeros(c)
# # print(np.shape(coef))
coef = np.load("./data/1stOrderFid/pcp,1stOrderSU3,coef,12.npy")
    
# res = minimize(lambda coef: subspace_variance(coef),method="SLSQP",x0=coef)
# coef = np.load("./pcp,1stOrderSU3,coef,14.npy")
...
# coef = coef[coef]
# np.save("pcp,1stOrderSU3,coef,"+str(pxp.N),coef)
# print(coef)
...
plt.show()
# np.save("pcp,0stOrder,energy,"+str(pxp.N),H.sector.eigvalues(k))
# np.save("pcp,0stOrder,overlap,"+str(pxp.N),overlap)
# np.save("pcp,0stOrder,fsa_energy,"+str(pxp.N),e)
# np.save("pcp,0stOrder,fsa_overlap,"+str(pxp.N),np.abs(u[0,:])**2)

t=np.arange(0,20,0.01)
f = fidelity(z,H,"use sym").eval(t,z)
# np.save("pcp,0stOrder,fidelity,"+str(pxp.N),f)
plt.plot(t,f)
plt.show()
...
plt.title(r"PCP SU(3) 2nd Order Corrections, N="+str(pxp.N))
plt.show()
# np.save("pcp,0stOrder,fsa_exact_overlap,"+str(pxp.N),exact_overlap)
