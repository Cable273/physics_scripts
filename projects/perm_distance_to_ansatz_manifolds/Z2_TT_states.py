...
def TT_wf(theta1,phi1,theta2,phi2):
    ...
    state = np.zeros(perm_dim,dtype=complex)
    for n in range(0,np.size(basis_refs,axis=0)):
        occs = perm_inv_key(basis_refs[n])
        coef = np.power(c1_down,pxp.N/2-occs[0])
        coef = coef * np.power(c1_up,occs[0])
        coef = coef * np.power(c2_down,pxp.N/2-occs[1])
        coef = coef * np.power(c2_up,occs[1])
        state[n] = coef
...
for n in pbar(range(0,np.size(evolved_states,axis=1))):
    if n == 0:
        res = minimize(lambda tt_params: TT_wf_distance(evolved_states[:,n],tt_params),method="nelder-mead",x0=[0,0,0,0])
        last_coef = res.x
    else:
