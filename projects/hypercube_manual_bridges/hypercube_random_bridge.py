...
# zero_index = dim
# H[:dim,:dim] = H0
# H[zero_index-1:,zero_index-1:] = np.flip(np.flip(H[:zero_index,:zero_index],axis=0),axis=1)

# H_orig = np.copy(H)
# print(np.shape(H_orig))
...
# # print((np.abs(H-H_orig)<1e-5).all())

# # e,u = np.linalg.eigh(H_orig)
# # psi_energy = np.conj(u[0,:])
# # t=np.arange(0,20,0.01)
# # f = np.zeros(np.size(t))
# # for n in range(0,np.size(t,axis=0)):
