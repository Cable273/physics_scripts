...
bridge_cube_H = dict()
cube_dims = dict()
for n in range(1,L-d+2):
    bridge_cube_H[2*n] = spin_chain_tb_H(2*n)
    cube_dims[2*n] = np.size(bridge_cube_H[2*n],axis=0)
...
plt.title(r"$\textrm{Cube Bridges Phenemological model, Equal bridge length}$, L="+str(L)+", d="+str(d))
plt.show()
