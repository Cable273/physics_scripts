...
for n in pbar(range(0,np.size(t,axis=0))):
    evolved_state_exact = time_evolve_state(z_energy,H.sector.eigvalues(),t[n])
    evolved_state_comp = np.dot(H.sector.eigvectors(),evolved_state_exact)

    evolved_state_cube_energy = time_evolve_state(neel_cube_energy,e_cube,t[n])
    evolved_state_perm_energy = time_evolve_state(neel_perm_energy,e_perm,t[n])
    ...
    cube_exact_overlap[n] = np.abs(np.vdot(evolved_state_comp,evolved_state_cube_comp))**2
    perm_exact_overlap[n] = np.abs(np.vdot(evolved_state_comp,evolved_state_perm_comp))**2
    fsa_exact_overlap[n] = np.abs(np.vdot(evolved_state_comp,evolved_state_fsa_comp))**2

plt.plot(t,cube_exact_overlap,label="Cube")
plt.plot(t,perm_exact_overlap,label="Perm")
plt.plot(t,fsa_exact_overlap,label="FSA")
plt.legend()
plt.xlabel(r"$t$")
plt.ylabel(r"$\vert \langle \psi_{approx}(t) \vert \psi_{exact}(t) \rangle \vert^2$")
plt.title(r"Time evolved overlap, N="+str(pxp.N))
plt.show()

# np.save("pxp,ppxp,subcube,t,16",t)
# np.save("pxp,ppxp,subcube,time_evolved,16",cube_exact_overlap)
# np.save("pxp,ppxp,fsa,time_evolved,16",fsa_exact_overlap)
# np.save("pxp,ppxp,perm,time_evolved,16",perm_exact_overlap)

np.save("pxp,subcube,t,16",t)
np.save("pxp,subcube,time_evolved,16",cube_exact_overlap)
np.save("pxp,fsa,time_evolved,16",fsa_exact_overlap)
np.save("pxp,perm,time_evolved,16",perm_exact_overlap)
