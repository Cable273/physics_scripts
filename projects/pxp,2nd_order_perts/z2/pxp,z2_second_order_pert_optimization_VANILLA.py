...
Hp[2].model = np.array([[0,-1,0,1,0],[0,1,0,-1,0],[0,-1,0,2,0],[0,2,0,-1,0]])
...
# coef = np.zeros(2)
# coef[0] = 0.108
coef = np.load("./data/vanilla_term/16/Vpxp,z2,2nd_order_perts,fid_coef,16.npy")
...
plt.title(r"$PXP, Z_2$, Second order pertubation (PXPIP), N="+str(pxp.N))
plt.show()
...
plt.title(r"$PXP, Z_2$, Second order pertubations (PXPIP), N="+str(pxp.N))
plt.show()
...
# np.savetxt("Vpxp,z2,2nd_order_perts,Hz_eigvalues,"+str(pxp.N),Hz_exp)
# np.savetxt("Vpxp,z2,2nd_order_perts,Hz_var,"+str(pxp.N),Hz_var)
# np.savetxt("Vpxp,z2,2nd_order_perts,Hz_eig_diffs,"+str(pxp.N),e_diff)
# np.save("Vpxp,z2,2nd_order_perts,fid_coef,"+str(pxp.N),coef)
# np.savetxt("Vpxp,z2,f0,"+str(pxp.N),[f0])
# np.savetxt("Vpxp,z2,spacing_error,"+str(pxp.N),[error])
