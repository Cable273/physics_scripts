...
coef = np.array([0.16515571,0.21071671])
# res = minimize(lambda coef:fidelity_error(coef),method="powell",x0=np.ones(2)*0.1)
res = minimize(lambda coef:fidelity_error(coef),method="powell",x0=coef)
# res = minimize(lambda coef:spacing_error(coef),method="powell",x0=np.ones(16)*0.1)
# coef = res.x
# print(coef)
# np.save("pxp,z3,2nd_order_pert,2_terms,coef,level_spacing,"+str(pxp.N),coef)
# coef = np.load("./level_cap_coef/pxp,z3,2nd_order_pert,coef,level_spacing,near_fid,15.npy")
coef = res.x
# coef = np.load("./pxp,z3,2nd_order_pert,coef,15.npy")
# coef = np.load("./pxp,z3,2nd_order_pert,coef,15.npy")
# coef = np.load("./level_cap_coef/pxp,z3,2nd_order_pert,coef,level_spacing,near_fid,15.npy")
print(coef)
# coef = np.load("./level_cap_coef/pxp,z3,2nd_order_pert,2_terms,coef,level_spacing,cap,15.npy")
...
np.savetxt("pxp,z3,f0,"+str(pxp.N),[f0])
...
plt.title(r"$PXP, Z_3$, Second order pertubations, optimal harmonic spacing, N="+str(pxp.N))
plt.show()
...
np.savetxt("pxp,z3,2nd_order_perts,ohs,Hz_eigvalues,"+str(pxp.N),e)
np.savetxt("pxp,z3,2nd_order_perts,ohs,Hz_var,"+str(pxp.N),var_vals)
np.savetxt("pxp,z3,2nd_order_perts,ohs,Hz_eig_diffs,"+str(pxp.N),e_diff)
