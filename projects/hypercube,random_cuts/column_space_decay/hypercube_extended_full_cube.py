...
#insert extra states, number from gaussian dist
...
#reflect+combine hypercube for full H
dim_final = 2*dim_total-dist[0]
H_final = np.zeros((dim_final,dim_final))

#first block
H_final[0:dim_total,0:dim_total] = H_total

print(np.size(H_total,axis=0))
print(np.size(pxp.basis_refs))
#second block, same as first with middle extra states removed
to_del = np.arange(np.size(pxp.basis_refs),np.size(pxp.basis_refs)+dist[0])
H_less = np.copy(H_total)
for n in range(np.size(to_del,axis=0)-1,-1,-1):
    H_less = np.delete(H_less,to_del[n],axis=0)
    H_less = np.delete(H_less,to_del[n],axis=1)

H_final[dim_total:,dim_total:] = H_less

plt.matshow(np.abs(H_final))
plt.show()

#insert extra couplings mirroring middle layer connections (copy blocks)
mid_extra_indices = np.arange(np.size(pxp.basis_refs),np.size(pxp.basis_refs)+dist[0])
left_extra_indices = np.arange(np.size(pxp.basis_refs)+dist[0]+1,np.size(pxp.basis_refs)+dist[0]+1+dist[1])
cube_indices = np.arange(0,np.size(pxp.basis_refs))

mid_cube_coupling  = H_final[np.size(pxp.basis_refs):(np.size(pxp.basis_refs)+dist[0]),0:np.size(pxp.basis_refs)]
mid_left_coupling  = H_final[np.size(pxp.basis_refs):(np.size(pxp.basis_refs)+dist[0]),(np.size(pxp.basis_refs)+dist[0]):(np.size(pxp.basis_refs)+dist[0]+dist[1])]

H_final[np.size(pxp.basis_refs):(np.size(pxp.basis_refs)+dist[0]),dim_total:(dim_total+np.size(pxp.basis_refs))] = mid_cube_coupling
H_final[np.size(pxp.basis_refs):(np.size(pxp.basis_refs)+dist[0]),(dim_total+np.size(pxp.basis_refs)):(dim_total+np.size(pxp.basis_refs)+dist[1])] = mid_left_coupling

H_final[dim_total:(dim_total+np.size(pxp.basis_refs)),np.size(pxp.basis_refs):(np.size(pxp.basis_refs)+dist[0])] = np.conj(np.transpose(mid_cube_coupling))
H_final[(dim_total+np.size(pxp.basis_refs)):(dim_total+np.size(pxp.basis_refs)+dist[1]),np.size(pxp.basis_refs):(np.size(pxp.basis_refs)+dist[0])] = np.conj(np.transpose(mid_left_coupling))

plt.matshow(np.abs(H_final))
plt.show()

#insert couplings from middle layer to new right layer, deleting duplicate middle layer
middle_hamming_refs = hamming_sectors[hamming_length-1]
left_hamming_refs = hamming_sectors[hamming_length-2]

middle_hamming_indices = np.zeros(np.size(middle_hamming_refs))
left_hamming_indices = np.zeros(np.size(left_hamming_refs))
for n in range(0,np.size(middle_hamming_refs,axis=0)):
    middle_hamming_indices[n] = pxp.keys[middle_hamming_refs[n]]
for n in range(0,np.size(left_hamming_refs,axis=0)):
    left_hamming_indices[n] = pxp.keys[left_hamming_refs[n]]

base_index = np.size(pxp.basis_refs)
for n in range(0,np.size(left_hamming_indices,axis=0)):
    for m in range(0,np.size(middle_hamming_indices,axis=0)):
        if H_final[int(left_hamming_indices[n]),int(middle_hamming_indices[m])] ==1:
            H_final[int(base_index + left_hamming_indices[n]),int(middle_hamming_indices[m])] = 1
            H_final[int(middle_hamming_indices[m]),int(base_index + left_hamming_indices[n])] = 1

#delete duplicate middle layer cube states
to_del = middle_hamming_indices + base_index
to_del=[]
for n in range(np.size(to_del,axis=0)-1,-1,-1):
    H_final = np.delete(H_final,to_del[n],axis=0)
    H_final = np.delete(H_final,to_del[n],axis=1)
...
# for n in pbar(range(0,np.size(e,axis=0))):
    ...
    f_max = np.max(f[cut:])
    plt.plot(t,f)
plt.title(r"$\textrm{Full Hypercube, Cycle Disorder}, \sigma = 1.5, f=2, R_{cube} = 1, R_{int} = 1, k = 2, N=12$")
plt.xlabel(r"$t$")
