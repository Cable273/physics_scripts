...
pxp = unlocking_System([0,1],"periodic",2,16)
pxp.gen_basis()
...
plt.title(r"$H=X+\lambda n n$, $\textrm{Neel state Fidelity}, \lambda = 0.1, N=16$")
plt.xlabel(r"$t$")
...
plt.title(r"$H=X+\lambda n n$, $\textrm{Eigenstate Overlap with Neel}, \lambda = 0.1, N=16$")
plt.xlabel(r"$E$")
