...
def group_perm_refs(sector):
    ...
    for m in range(0,np.size(refs,axis=0)):
        ...
        goes_to_refs = np.zeros(np.size(goes_to,axis=0))
        for k in range(0,np.size(goes_to_refs,axis=0)):
            goes_to_refs[k] = bin_to_int_base_m(goes_to[k],int(pxp.N/2+1))
        max_number = (pxp.N/2)+pxp.N/2 + pxp.N/2
        # sector_ref = bin_to_int_base_m(goes_to_refs,max_number)
        sector_ref = np.size(goes_to,axis=0)
...
#form basis of identical states from perm sectors
basis = np.zeros(pxp.dim)
for n in range(0,np.size(perm_sectors,axis=0)):
    uniq_refs = group_perm_refs(perm_sectors[n])
    keys = list(uniq_refs.keys())
    for m in range(0,np.size(keys)):
        block_refs = uniq_refs[keys[m]]
        temp_state = np.zeros(pxp.dim)
        for k in range(0,np.size(block_refs,axis=0)):
            temp_state[pxp.keys[block_refs[k]]] = 1
