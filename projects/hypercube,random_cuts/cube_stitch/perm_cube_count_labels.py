    ...
    for m in range(0,np.size(refs,axis=0)):
        ...
        for k in range(0,np.size(bits)):
            if k == np.size(bits)-1:
                ...
                kp4 = 3
                kp5 = 4
            elif k == np.size(bits)-2:
                kp1 = k + 1
                kp2 = 0
                kp3 = 1
                kp4 = 2
                kp5 = 3
            elif k == np.size(bits)-3:
                kp1 = k + 1
                kp2 = k + 2
                kp3 = 0
                kp4 = 1
                kp5 = 2
            elif k == np.size(bits)-4:
                kp1 = k + 1
                kp2 = k + 2
                kp3 = k + 3
                kp4 = 0
                kp5 = 1
            elif k == np.size(bits)-5:
                kp1 = k + 1
                kp2 = k + 2
                kp3 = k + 3
                kp4 = k + 4
                ...
                kp4 = k + 4
                kp5 = k + 5
            # if bits[k] == 0 and bits[kp1] == 0 and bits[kp2] == 0 and bits[kp3] ==0 and bits[kp4] == 0 and bits[kp5] == 0:
                # a = a + 1
            # if bits[k] == 0 and bits[kp1] == 0 and bits[kp2] == 0 and bits[kp3] ==0 and bits[kp4] == 0:
                # b = b + 1
            if bits[k] == 0 and bits[kp1] == 0 and bits[kp2] == 0 and bits[kp3] ==0: 
                b = b + 1
            # if bits[k] == 0 and bits[kp1] == 0 and bits[kp2] == 0: 
                # d = d + 1

            k_current = k
            if bits[k] == 0:
                zero_sum = 1
                for i in range(0,pxp.N):
                    if k_current == pxp.N-1:
                        kp1 = 0
                    else:
                        kp1 = k_current + 1
            ...
            if zero_sum > longest_zeros_string:
                longest_zeros_string = zero_sum
        ...
        # print(perm_sectors[n],c_flip,b_flip,bits)
        sector_label = np.append(perm_sectors[n],np.array([c_flip,b_flip]))
        # sector_label = np.append(perm_sectors[n],[b])
        # print(bits,longest_zeros_string)
        # sector_label = np.append(perm_sectors[n],[longest_zeros_string])
...
plt.scatter(eigenvalues,overlap,marker="s",s=200,alpha=0.6,color="red",label="Perm, 0000 string count")
...
to_del=[]
for n in range(0,np.size(overlap_fsa,axis=0)):
    if overlap_fsa[n] <-5:
        to_del = np.append(to_del,n)
for n in range(np.size(to_del,axis=0)-1,-1,-1):
    overlap_fsa=np.delete(overlap_fsa,to_del[n])
    eigenvalues=np.delete(eigenvalues,to_del[n])
    
plt.scatter(ef,overlap_fsa,marker="D",s=100,color="green",alpha =0.6,label="FSA")
plt.legend()
plt.title("Scar approximations, overlap with Neel state, N=12")
plt.show()
...
print(exact_overlap)
plt.scatter(e,exact_overlap,marker="s",color="red",s=100,alpha=0.6,label="Perm, 0000 String Count")
...
plt.title("Scar approximations, overlap with exact eigenstates, N=12")
plt.legend()
plt.show()
